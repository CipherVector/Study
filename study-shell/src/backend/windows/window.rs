...
/// (because if it did, the system could call back into study-shell with some mouse event, and then
...
/// 1. the system hands a mouse click event to study-shell
/// 2. study-shell calls `WinHandler::mouse_up`
...
/// 5. study-shell displays the "save as" dialog that was requested in step 3.
enum DeferredOp {
