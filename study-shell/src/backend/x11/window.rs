...
// 2) from the client (e.g. study, calling a method on its `WindowHandle`).
//
    ...
    pub fn handle_button_release(
        ...
        let mouse_event = MouseEvent {
            pos: Point::new(button_release.event_x as f64, button_release.event_y as f64)
                .to_dp(scale),
            // The xcb state includes the newly released button, but study
            // doesn't want it.
