...
const BASE_DRUID_DEPRECATED_ATTR_PATH: &str = "study";
    ...
    pub fn parse_ast(field: &syn::Field, index: usize) -> Result<Self, Error> {
        ...
        for attr in field.attrs.iter() {
            if attr.path.is_ident(BASE_DRUID_DEPRECATED_ATTR_PATH) {
                panic!(
                    "The 'study' attribute has been replaced with separate \
    ...
    pub fn same_fn_path_tokens(&self) -> TokenStream {
        match &self.attrs {
            ...
            DataAttr::Empty => {
                ...
                quote_spanned!(span=> study::Data::same)
            }
        }
    }
}
