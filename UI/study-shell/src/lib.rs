...
//! Platform abstraction for study toolkit.
//!
//! `study-shell` is an abstraction around a given platform UI & application
//! framework. It provides common types, which then defer to a platform-defined
//! implementation.
//!
//! # Env
//!
//! For testing and debugging, `study-shell` can change its behavior based on environment
//! variables. Here is a list of environment variables that `study-shell` supports:
//!
//! - `DRUID_SHELL_DISABLE_X11_PRESENT`: if this is set and `study-shell` is using the `x11`
