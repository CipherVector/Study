...
use study::text::{AttributesAdder, RichText, RichTextBuilder};
use study::widget::prelude::*;
use study::widget::{Controller, LineBreaking, RawLabel, Scroll, Split, TextBox};
use study::{
...
const TEXT: &str = "*Hello* ***world***! This is a `TextBox` where you can \
		    ...
		    **styling** of the `Label` on the left. ~~Strikethrough even works!~~\n\n\
		    ...
		    in the #study-help and #study channels, respectively.\n\n\n\
...
const OPEN_LINK: Selector<String> = Selector::new("study-example.open-link");
...
fn make_menu<T: Data>(_window_id: Option<WindowId>, _app_state: &AppState, _env: &Env) -> Menu<T> {
    ...
    #[cfg(target_os = "macos")]
    {
        base = base.entry(study::platform_menus::mac::application::default())
    }
    #[cfg(any(target_os = "windows", target_os = "linux", target_os = "openbsd"))]
    {
        base = base.entry(study::platform_menus::win::file::default());
    }
    base.entry(
        Menu::new(LocalizedString::new("common-menu-edit-menu"))
            .entry(study::platform_menus::common::undo())
            .entry(study::platform_menus::common::redo())
            .separator()
            .entry(study::platform_menus::common::cut().enabled(false))
            .entry(study::platform_menus::common::copy())
            .entry(study::platform_menus::common::paste()),
    )
}
