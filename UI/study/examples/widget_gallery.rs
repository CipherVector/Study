...
fn load_xi_image<Ctx: study::RenderContext>(ctx: &mut Ctx) -> Ctx::Image {
    ctx.make_image(32, 32, XI_IMAGE, study::piet::ImageFormat::Rgb)
        .unwrap()
}
...
fn paint_example<T>(ctx: &mut PaintCtx, _: &T, _env: &Env) {
    ...
    ctx.draw_image(
        &img,
        bounds,
        study::piet::InterpolationMode::NearestNeighbor,
    );
    ctx.with_save(|ctx| {
