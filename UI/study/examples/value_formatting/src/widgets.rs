...
use study::text::ValidationError;
use study::widget::{
    prelude::*, Controller, Either, Label, SizedBox, TextBoxEvent, ValidationDelegate,
};
use study::{Color, Data, Point, Selector, WidgetExt, WidgetId, WidgetPod};
...
const SHOW_ERROR: Selector<ValidationError> = Selector::new("study-example.show-error");
/// Sent by the [`TextBoxErrorDelegate`] when an error should be cleared.
const CLEAR_ERROR: Selector = Selector::new("study-example.clear-error");
/// Sent by the [`TextBoxErrorDelegate`] when editing began.
///
/// This is used to set the contents of the help text.
const EDIT_BEGAN: Selector<WidgetId> = Selector::new("study-example.edit-began");
/// Sent by the [`TextBoxErrorDelegate`] when editing finishes.
///
/// This is used to set the contents of the help text.
const EDIT_FINISHED: Selector<WidgetId> = Selector::new("study-example.edit-finished");
...
pub fn explainer() -> impl Widget<AppData> {
    Label::dynamic(|d: &Option<&'static str>, _| d.unwrap_or(DEFAULT_EXPLAINER).to_string())
        .with_line_break_mode(study::widget::LineBreaking::WordWrap)
        .with_text_color(study::theme::FOREGROUND_DARK)
        .with_font(study::theme::UI_FONT_ITALIC)
