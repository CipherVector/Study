...
impl TextComponent<()> {
    ...
    /// a focus change.
    pub const TAB: Selector = Selector::new("study-builtin.textbox-tab");
    ...
    /// An ancestor can handle this event in order to do things like request
    /// a focus change.
    pub const BACKTAB: Selector = Selector::new("study-builtin.textbox-backtab");
}
    ...
    fn do_action(&mut self, buffer: &mut T, action: TextAction) {
        match action {
            ...
            TextAction::Delete(movement) if self.selection.is_caret() => {
                if movement == Movement::Grapheme(study_shell::text::Direction::Upstream) {
...
impl<T: TextStorage + EditableText> InputHandler for EditSessionHandle<T> {
    ...
    fn line_range(&self, index: usize, _affinity: study_shell::text::Affinity) -> Range<usize> {
