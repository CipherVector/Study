...
///
/// Certain `Selector`s are defined by study, and have special meaning
/// to the framework; these are listed in the [`study::commands`] module.
///
/// [`Command`]: struct.Command.html
/// [`Command::get`]: struct.Command.html#method.get
/// [`get_unchecked`]: struct.Command.html#method.get_unchecked
/// [`study::commands`]: commands/index.html
...
/// # Examples
/// ```
/// use study::{Command, Selector, Target};
///
...
/// # Examples
/// ```
/// use study::{Command, Selector, SingleUse, Target};
///
/// struct CantClone(u8);
///
...
/// Commands with special meaning, defined by study.
///
...
pub mod sys {
    ...
    /// Quit the running application. This command is handled by the study library.
    pub const QUIT_APP: Selector = Selector::new("study-builtin.quit-app");
    ...
    pub const HIDE_APPLICATION: Selector = Selector::new("study-builtin.menu-hide-application");
    ...
    pub const HIDE_OTHERS: Selector = Selector::new("study-builtin.menu-hide-others");

    /// The selector for a command to create a new window.
    pub(crate) const NEW_WINDOW: Selector<SingleUse<Box<dyn Any>>> =
        Selector::new("study-builtin.new-window");
    ...
    pub const CLOSE_WINDOW: Selector = Selector::new("study-builtin.close-window");

    /// Close all windows.
    pub const CLOSE_ALL_WINDOWS: Selector = Selector::new("study-builtin.close-all-windows");
    ...
    /// When calling `submit_command` on a `Widget`s context, passing `None` as target
    /// will automatically target the window containing the widget.
    pub const SHOW_WINDOW: Selector = Selector::new("study-builtin.show-window");
    ...
    pub const CONFIGURE_WINDOW: Selector<WindowConfig> =
        Selector::new("study-builtin.configure-window");
    ...
    pub(crate) const SHOW_CONTEXT_MENU: Selector<SingleUse<Box<dyn Any>>> =
        Selector::new("study-builtin.show-context-menu");
    ...
    pub(crate) const NEW_SUB_WINDOW: Selector<SingleUse<SubWindowDesc>> =
        Selector::new("study-builtin.new-sub-window");
    ...
    pub(crate) const SUB_WINDOW_PARENT_TO_HOST: Selector<SubWindowUpdate> =
        Selector::new("study-builtin.parent_to_host");
    ...
    pub(crate) const SUB_WINDOW_HOST_TO_PARENT: Selector<Box<dyn Any>> =
        Selector::new("study-builtin.host_to_parent");

    /// Show the application preferences.
    pub const SHOW_PREFERENCES: Selector = Selector::new("study-builtin.menu-show-preferences");

    /// Show the application about window.
    pub const SHOW_ABOUT: Selector = Selector::new("study-builtin.menu-show-about");

    /// Show all applications.
    pub const SHOW_ALL: Selector = Selector::new("study-builtin.menu-show-all");

    /// Show the new file dialog.
    pub const NEW_FILE: Selector = Selector::new("study-builtin.menu-file-new");
    ...
    pub const SHOW_OPEN_PANEL: Selector<FileDialogOptions> =
        Selector::new("study-builtin.menu-file-open");

    /// Sent when the user cancels an open file panel.
    pub const OPEN_PANEL_CANCELLED: Selector = Selector::new("study-builtin.open-panel-cancelled");

    /// Open a path, must be handled by the application.
    ///
    /// [`FileInfo`]: ../struct.FileInfo.html
    pub const OPEN_FILE: Selector<FileInfo> = Selector::new("study-builtin.open-file-path");

    /// Open a path, must be handled by the application.
    ///
    /// [`FileInfo`]: ../struct.FileInfo.html
    pub const OPEN_FILES: Selector<Vec<FileInfo>> = Selector::new("study-builtin.open-files-path");
    ...
    pub const SHOW_SAVE_PANEL: Selector<FileDialogOptions> =
        Selector::new("study-builtin.menu-file-save-as");

    /// Sent when the user cancels a save file panel.
    pub const SAVE_PANEL_CANCELLED: Selector = Selector::new("study-builtin.save-panel-cancelled");
    ...
    pub const SAVE_FILE: Selector<()> = Selector::new("study-builtin.save-file");

    /// Save to a given location.
    ///
    /// This command is emitted by study whenever a save file dialog successfully completes. The
    ...
    /// expectations.
    pub const SAVE_FILE_AS: Selector<FileInfo> = Selector::new("study-builtin.save-file-as");

    /// Show the print-setup window.
    pub const PRINT_SETUP: Selector = Selector::new("study-builtin.menu-file-print-setup");

    /// Show the print dialog.
    pub const PRINT: Selector = Selector::new("study-builtin.menu-file-print");

    /// Show the print preview.
    pub const PRINT_PREVIEW: Selector = Selector::new("study-builtin.menu-file-print");

    /// Cut the current selection.
    pub const CUT: Selector = Selector::new("study-builtin.menu-cut");

    /// Copy the current selection.
    pub const COPY: Selector = Selector::new("study-builtin.menu-copy");

    /// Paste.
    pub const PASTE: Selector = Selector::new("study-builtin.menu-paste");

    /// Undo.
    pub const UNDO: Selector = Selector::new("study-builtin.menu-undo");

    /// Redo.
    pub const REDO: Selector = Selector::new("study-builtin.menu-redo");

    /// Select all.
    pub const SELECT_ALL: Selector = Selector::new("study-builtin.menu-select-all");

    /// Text input state has changed, and we need to notify the platform.
    pub(crate) const INVALIDATE_IME: Selector<ImeInvalidation> =
        Selector::new("study-builtin.invalidate-ime");
    ...
    pub const SCROLL_TO_VIEW: Selector<Rect> = Selector::new("study-builtin.scroll-to");
    ...
    pub fn get<T: Any>(&self, selector: Selector<T>) -> Option<&T> {
        if self.symbol == selector.symbol() {
            Some(self.payload.downcast_ref().unwrap_or_else(|| {
                panic!(
                    "The selector \"{}\" exists twice with different types. See study::Command::get for more information",
                    selector.symbol()
                );
            }))
        } else {
        ...
        if self.symbol == selector.symbol() {
            Some(self.payload.downcast_ref().unwrap_or_else(|| {
                panic!(
                    ...
                    See study::Command::get for more information",
                    selector.symbol()
                );
            }))
        } else {
