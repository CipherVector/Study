...
///
/// In `study`, unlike in `study_shell`, we treat the widget's coordinate
...
impl From<study_shell::MouseEvent> for MouseEvent {
    fn from(src: study_shell::MouseEvent) -> MouseEvent {
        let study_shell::MouseEvent {
            pos,
            buttons,
            mods,
            count,
