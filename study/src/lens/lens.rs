...
/// use [`#[derive(Lens)]`](study_derive::Lens) to access a struct's
...
pub trait LensExt<A: ?Sized, B: ?Sized>: Lens<A, B> {
    ...
    ///
    /// ```
    /// use study::{widget::TextBox, Data, Lens, LensExt, Widget, WidgetExt};
    ///
...
///
/// ```
/// let lens = study::lens::Field::new(|x: &Vec<u32>| &x[42], |x| &mut x[42]);
/// ```
...
/// struct Foo { x: Bar }
/// struct Bar { y: [i32; 10] }
/// let lens = study::lens!(Foo, x);
/// let lens = study::lens!((u32, bool), 1);
/// let lens = study::lens!([u8], [4]);
/// let lens = study::lens!(Foo, x.y[5]);
/// ```
#[macro_export]
macro_rules! lens {
    ($ty:ty, [$index:expr]) => {
