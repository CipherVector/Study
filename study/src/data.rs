...
/// can be expensive. To use collection types with study, there are two easy options:
/// either wrap the collection in an `Arc`, or build study with the `im` feature,
/// which adds `Data` implementations to the collections from the [`im` crate],
/// a set of immutable data structures that fit nicely with study.
///
...
impl_data_simple!(study::piet::InterpolationMode);
#[cfg(feature = "chrono")]
