...
const BEGIN_EDITING: Selector = Selector::new("study.builtin.textbox-begin-editing");
const COMPLETE_EDITING: Selector = Selector::new("study.builtin.textbox-complete-editing");
    ...
    fn update(&mut self, ctx: &mut UpdateCtx, old: &T, data: &T, env: &Env) {
        ...
        if self.is_editing {
            if changed_by_us {
                ...
                // study, data can change anywhere in the tree. If we are actively
