...
//! The implementation of the WinHandler trait (study-shell integration).
...
/// The struct implements the study-shell `WinHandler` trait.
///
...
///
/// This corresponds to the `AppHandler` trait in study-shell, which is only
...
/// The information for forwarding study-shell's file dialog reply to the right place.
struct DialogInfo {
...
impl<T: Data> InnerAppState<T> {
    ...
    fn do_update(&mut self) {
        ...
        #[cfg(target_os = "macos")]
        {
            use study_shell::platform::mac::ApplicationExt as _;
...
impl<T: Data> AppState<T> {
    ...
    /// Handle a 'command' message from study-shell. These map to  an item
    ...
    fn new_window(&mut self, cmd: Command) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // The NEW_WINDOW command is private and only study can receive it by normal means,
...
impl<T: Data> WinHandler for DruidHandler<T> {
    ...
    fn mouse_down(&mut self, event: &MouseEvent) {
        // TODO: double-click detection (or is this done in study-shell?)
