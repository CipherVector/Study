...
```
This shows some of the basics of study. If you need a start of how to build an application with a text-box and some labels this is where to start.

## Hello_web
For more info and prerequistes see [study/examples/hello_web/README.md](study/examples/hello_web/README.md).
```
cd study/examples/hello_web
wasm-pack build --out-dir pkg --out-name hello_web
```
[View at http://localhost:8000](http://localhost:8000).

This is an example of how to get almost any study application can be used on the web. This is just the hello_world example but should work for all of them.

## Web
For more info and prerequistes see [study/examples/web/README.md](study/examples/web/README.md).
```
cd study/examples/web
...
```
An example of how basic widget composition works in study. There are no custom widgets just compositions of builtin ones.

## Lens
```
cargo run --example lens
```
Lenses are a core part of study, they allow you to zoom into a part of the app state.
...
To run this example, make sure you are in `study/examples/value_formatting`
...
```
Switches are useful in many ways, this example shows how to use the study built-in ones. This includes on/off and up/down for incrementing numeric values.

## Tabs
```
cargo run --example tabs --features="im"
```
Tabs allow you to seperate different portions of the UI. This example shows you how to use them in study. similar to [view switcher](#View Switcher) but with with a different purpose.
...
Please note that the image is exported with some kind of interpolation. So even when you turn interpolation off/NearestNeighbor in study, you will still see this because that's how the image actually looks.
...
In study you can change all kinds of styling aspects of text as not all text should look
