...
use study::widget::{Align, Button, Flex, TextBox};
use study::{
...
fn ui_builder() -> impl Widget<String> {
    ...
    let save = Button::new("Save").on_click(move |ctx, _, _| {
        ctx.submit_command(study::commands::SHOW_SAVE_PANEL.with(save_dialog_options.clone()))
    });
    let open = Button::new("Open").on_click(move |ctx, _, _| {
        ctx.submit_command(study::commands::SHOW_OPEN_PANEL.with(open_dialog_options.clone()))
    });
